...
const DashboardAntenne = ({items, getListStyle, clickAudio, removeMessageLive, folder}) => {
    return(
        <div className="dashboard-antenne">
            ...
            <div className="dashboard-antenne_body">
                <div className="dashboard-title text-black">
                    ...
                    {items.length > 0 && (
                        <a href="#" className='dashboard-antenne_remove' onClick={removeMessageLive}>Retirer la réaction</a>
                    )}
                </div>
                ...
                <div className="dashboard-folders">
                    ...
                    <div className="dashboard-folders_container">
                        {folder.map(item => (
                            <Droppable droppableId={`droppable-folder${item.id}`} key={item.id}>
                                {(provided, snapshot) => (
                                    <div
                                        {...provided.droppableProps}
                                        ref={provided.innerRef}
                                        style={getListStyle(snapshot.isDraggingOver)}
                                        className={`dashboard-folders_item ${item.id === 0 && 'first'}`}
                                    >
                                            <article className={item.count > 0 ? 'active' : ''}>
                                                <p>{item.text}</p>
                                            </article>
                                        {provided.placeholder}
                                    </div>
